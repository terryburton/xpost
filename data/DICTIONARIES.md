...
|---|---|---|---|---|
| `.xpostint` | global | static | read-only + anchor dropped | frozen `//.xpostint /h get exec`; the home for helpers that embed a local reference |
...
`.xpostint` and `.xpostsys` are both global because a global `systemdict`
...
2. **Determine the access.**
   ...
   - Interpreter-private machinery, static, reached only by the machinery →
     `.xpostsys` (or `.xpostint` if it must freeze a local reference), where it
